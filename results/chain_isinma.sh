...
nohup python -u scripts/policy_server.py --ckpt "$CKPT" --device cuda \
  ...
  > "$R/server_isinma.log" 2>&1 &
SRV=$!
for i in $(seq 1 40); do grep -q "dinleniyor" "$R/server_isinma.log" 2>/dev/null && break; sleep 10; done
conda activate isaaclab
timeout 2400 python -u scripts/eval_policy_isaacsim.py --num_envs 8 \
  --num_episodes 20 --env_spacing 25.0 --headless \
  --dump_obs "$R/live_obs_isinma.npz" --dump_step 2 \
  --kit_args="--/rtx/verifyDriverVersion/enabled=false" \
  > "$R/eval_isinma.log" 2>&1
kill $SRV 2>/dev/null; sleep 8
log "ISINMA DUZELTMESI OLCULDU"
