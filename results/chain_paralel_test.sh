...
nohup python -u scripts/policy_server.py --ckpt "$CK" --device cuda \
  ...
  > "$R/server_ptest.log" 2>&1 &
SRV=$!
for i in $(seq 1 40); do grep -q "dinleniyor" "$R/server_ptest.log" 2>/dev/null && break; sleep 5; done
conda activate isaaclab
timeout 1800 python -u scripts/eval_policy_isaacsim.py --num_envs 8 \
  --num_episodes 16 --env_spacing 25.0 --headless --fix_cam \
  --kit_args="--/rtx/verifyDriverVersion/enabled=false" \
  > "$R/eval_ptest.log" 2>&1
kill $SRV 2>/dev/null; sleep 5
log "PARALEL TEST: $(grep -a 'SONUC' $R/eval_ptest.log | tail -1)"
