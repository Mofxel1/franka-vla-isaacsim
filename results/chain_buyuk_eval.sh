...
run () {  # $1=ckpt $2=etiket
  ...
  timeout 4200 python -u scripts/eval_policy_isaacsim.py --num_envs 8 \
    --num_episodes 60 --env_spacing 25.0 --headless --fix_cam \
    --kit_args="--/rtx/verifyDriverVersion/enabled=false" \
    > "$R/eval_$2.log" 2>&1
  kill $SRV 2>/dev/null; sleep 8
  log "BUYUK EVAL: $2 -> $(grep -a 'SONUC' $R/eval_$2.log | tail -1)"
}
run ~/franka_runs/train_fixcam/checkpoints/004000/pretrained_model b60_fixcam
run ~/franka_runs/train_fixdag/checkpoints/004000/pretrained_model b60_fixdag
log "BUYUK EVAL TAMAMLANDI"
