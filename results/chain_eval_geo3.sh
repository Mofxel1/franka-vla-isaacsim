...
CK=~/franka_runs/train_geo3/checkpoints/006000/pretrained_model
...
nohup python -u scripts/policy_server.py --ckpt "$CK" --device cuda \
  --object_centric --n_action_steps 25 --n_samples 8 --port 8765 > "$R/server_geo3.log" 2>&1 &
SRV=$!
for i in $(seq 1 40); do grep -q "dinleniyor" "$R/server_geo3.log" 2>/dev/null && break; sleep 10; done
conda activate isaaclab
timeout 3000 python -u scripts/eval_policy_isaacsim.py --num_envs 8 \
  --num_episodes 20 --env_spacing 25.0 --headless \
  --kit_args="--/rtx/verifyDriverVersion/enabled=false" \
  --out_gif "$R/eval_geo3.gif" > "$R/eval_geo3.log" 2>&1
kill $SRV 2>/dev/null
log "GEO3 KAPALI DONGU HAZIR"
