...
O=~/franka_runs/train_objc2
ROOT=/home/orhan/franka_runs/lerobot/franka_lift_objc2
BASE=~/franka_runs/train_objc/checkpoints/006000/pretrained_model
...
d=LeRobotDataset('franka_lift_objc2', root='$ROOT')
sys.exit(0 if d.num_episodes>900 else 1)" 2>/dev/null && break
  sleep 30
done
log "objc2 (skip_first=2) veri seti hazir"
rm -rf "$O"
lerobot-train --dataset.repo_id=franka_lift_objc2 --dataset.root="$ROOT" \
  ...
  --num_workers=2 --seed=1000 > "$R/train_objc2.log" 2>&1
log "objc2 egitimi bitti"
{
echo "===== TEMIZ KARE (skip_first=2) + NESNE-MERKEZLI ====="
...
nohup python -u scripts/policy_server.py --ckpt "$BEST" --device cuda \
  --object_centric --n_action_steps 25 --n_samples 8 --port 8765 > "$R/server_objc2.log" 2>&1 &
SRV=$!
for i in $(seq 1 40); do grep -q "dinleniyor" "$R/server_objc2.log" 2>/dev/null && break; sleep 10; done
conda activate isaaclab
timeout 3000 python -u scripts/eval_policy_isaacsim.py \
  --num_envs 8 --num_episodes 20 --env_spacing 25.0 --headless \
  --kit_args="--/rtx/verifyDriverVersion/enabled=false" \
  --out_gif "$R/eval_objc2.gif" > "$R/eval_objc2.log" 2>&1
kill $SRV 2>/dev/null
log "OBJC2 SONUC HAZIR"
